<?php
/*
Plugin Name: WP-PageNavi-LightWord
Version: 1,0
Description: Adapts WP-PageNavi for LightWord wordpress theme
Author: Agus Suhanto, Lester 'GaMerZ' Chan & scribu
Author URI: http://suhanto.net/
Plugin URI: http://suhanto.net/wp-pagenavi-lightword/
Text Domain: wp-pagenavi
Domain Path: /lang


Copyright 
   * 2010  Agus Suhanto (email : agus@suhanto.net)
   * 2009  Lester Chan  (email : lesterchan@gmail.com)
	...
	switch ( intval($options['style']) ) {
		...
		case 1:
			if ( !empty($options['pages_text']) ) {
				...
				$options['pages_text']);
				$out .= "<div class='pages'>$pages_text</div>";
			}
			...
			if ( $start_page >= 2 && $pages_to_show < $max_page ) {
				...
				if ( !empty($options['dotleft_text']) )
					$out .= "<div class='extend'>{$options['dotleft_text']}</div>";
			}
            ...
            $prevlink = get_previous_posts_link($options['prev_text']);
			if ( !empty($options['prev_text']) && strlen(trim($prevlink)) > 0 )
				$out .= str_replace($options['prev_text'], "<div class='prevdiv'>".$options['prev_text']."</div>", $prevlink);

			for ( $i = $start_page; $i <= $end_page; $i++) {
				if ( $i == $paged && !empty($options['current_text']) ) {
					$current_page_text = str_replace('%PAGE_NUMBER%', number_format_i18n($i), $options['current_text']);
					$out .= "<div class='current'>$current_page_text</div>";
				} else {
					$out .= _wp_pagenavi_single($i, 'page', $options['page_text']);
				}
			}

			$nextlink = get_next_posts_link($options['next_text'], $max_page);
			if ( !empty($options['next_text']) && strlen(trim($nextlink)) > 0)
				$out .= str_replace($options['next_text'], "<div class='nextdiv'>".$options['next_text']."</div>", $nextlink);
			...
			if ( $end_page < $max_page ) {
				if ( !empty($options['dotright_text']) )
					$out .= "<div class='extend'>{$options['dotright_text']}</div>";
...
function _wp_pagenavi_single($page, $class, $raw_text, $format = '%PAGE_NUMBER%') {
	...
	return "<a href='" . esc_url(get_pagenum_link($page)) . "' class='$class'><div class='$class'>$text</div></a>";
}
